...
test('normalizes exact HTTP(S) origins and rejects unsafe remote URLs', () => {
  assert.equal(normalizeRemoteServerUrl(' HTTPS://Example.COM:443/ '), 'https://example.com');
  assert.equal(normalizeRemoteServerUrl('http://example.com:80/'), 'http://example.com');
...
test('health probes are explicit, credentialless, redirect-safe, and fail closed', async () => {
  ...
  await assert.rejects(probeRemoteServer('https://probe.example/', {
    fetchImpl: async () => ({ status: 200, redirected: false, json: async () => { throw new SyntaxError('bad JSON'); } }),
  }), /malformed JSON/);
  await assert.rejects(probeRemoteServer('https://probe.example/', {
